var chat = new function(){
	...
	function getChatHistory(callback){
		...
		var chats = [];
		chats.push({datetime:new Date(d.setTime(d.getTime()+2000)), message:"hello", from:"Visitor"});
		chats.push({datetime:new Date(d.setTime(d.getTime()+4000)), message:"Hi, how can I help you?", from:"Operator"});
		chats.push({datetime:new Date(d.setTime(d.getTime()+4000)), message:"I'm looking for a size 7, but can't find it", from:"Visitor"});
		chats.push({datetime:new Date(d.setTime(d.getTime()+4000)), message:"Ok, one moment I'll check the stock", from:"Operator"})
		chats.push({datetime:new Date(d.setTime(d.getTime()+10000)), message:"I'm sorry, there is no sie 7 available in that colour. There are some in red and blue however", from:"Operator"})
		chats.push({datetime:new Date(d.setTime(d.getTime()+4000)), message:"Oh great, thank you", from:"Visitor"});
		chats.push({datetime:new Date(d.setTime(d.getTime()+4000)), message:"my pleasure :-)", from:"Operator"});
	...
	function dispatchChatEvent(msg, from){
		var event = new CustomEvent('chatreceived', {"detail":{datetime:new Date(), message:msg, from:from}});

		// Listen for the event
		//chat.addEventListener('chatreceived', function (e) { console.log(e.detail) }, false);

		// Dispatch the event.
		raiseEvent("chatreceived", {"chat":{datetime:new Date(), message:msg, from:from}});
	}
